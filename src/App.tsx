...
function MainPOS() {
  ...
  if (!userRole) {
    return (
      <div style={{ display: 'flex', height: '100vh', width: '100%', alignItems: 'center', justifyContent: 'center', background: '#0b1120', fontFamily: 'sans-serif', margin: 0 }}>
        <style>{`html, body, #root { width: 100%; height: 100vh; overflow: hidden; margin: 0; padding: 0; }`}</style>
        <div style={{ background: '#1e293b', padding: '40px 30px', borderRadius: '24px', textAlign: 'center', width: '90%', maxWidth: '400px', border: '1px solid #334155' }}>
          <h1 style={{ fontSize: '45px', margin: '0 0 15px 0' }}>🏪</h1>
          <h2 style={{ margin: '0 0 10px 0', color: '#f8fafc' }}>Kashif Bakery</h2>
          <form onSubmit={handleLogin}>
            <input type="password" placeholder="PIN" value={loginPin} onChange={e => setLoginPin(e.target.value)} style={{ width: '100%', padding: '15px', fontSize: '20px', textAlign: 'center', letterSpacing: '10px', borderRadius: '12px', border: '1px solid #475569', background: '#0f172a', color: '#fff', marginBottom: '25px', outline: 'none' }} autoFocus />
            <button type="submit" style={{ width: '100%', padding: '16px', fontSize: '16px', background: '#3b82f6', color: '#fff', border: 'none', borderRadius: '12px', cursor: 'pointer', fontWeight: '700' }}>🔓 Unlock</button>
          </form>
        </div>
      </div>
    );
  ...
  return (
    <div className={`app-container ${darkMode ? 'dark-theme' : 'light-theme'}`}>
      ...
      {showInquiry && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.8)', zIndex: 5000, display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '15px' }}>
          <div style={{ background: 'var(--card-bg)', padding: '20px', borderRadius: '16px', width: '100%', maxWidth: '600px', border: '1px solid var(--border-color)', display: 'flex', flexDirection: 'column', maxHeight: '90vh' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '15px' }}>
              <h2 className="sharp-heading" style={{fontSize:'18px'}}>🔍 Search</h2>
      ...
      {receiptData && (
        <div className="receipt-overlay" style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(15, 23, 42, 0.85)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 9999, padding: '15px', backdropFilter: 'blur(4px)' }}>
          <div className="receipt-box" style={{ backgroundColor: '#fff', padding: '20px', width: '100%', maxWidth: '350px', borderRadius: '16px', fontFamily: 'monospace', color: '#000', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ marginBottom: '15px' }}>{(receiptData.items || []).map((item, idx) => ( <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px', fontSize: '12px' }}><span>{item.product_name || item.name} (x{item.qty})</span><span style={{ fontWeight: 'bold' }}>Rs. {((item.price || 0) * (item.qty || 0)).toLocaleString()}</span></div> ))}</div>
...
export default function App() { return <AppErrorBoundary><MainPOS /></AppErrorBoundary>; }